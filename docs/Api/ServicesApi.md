...
# **listServices**
> \KuntaAPI\Model\Service[] listServices($organizationId, $search, $sortOrder, $sortDir, $firstResult, $maxResults)
...
try {
    $result = $api_instance->listServices($organizationId, $search, $sortOrder, $sortDir, $firstResult, $maxResults);
    print_r($result);
} catch (Exception $e) {
